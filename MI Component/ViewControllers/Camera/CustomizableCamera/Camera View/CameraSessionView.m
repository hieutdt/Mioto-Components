...
-(void)composeInterface {
    ...
    if (_topBarView) {
        ...
        if (_cameraFlash) {
            ...
            if ( [[UIDevice currentDevice] userInterfaceIdiom] != UIUserInterfaceIdiomPad ) [_topBarView addSubview:_cameraFlash];
        }
    ...
    // Layout shutter button by orientation
    [self layoutShutterButtonByOrientation];
    
}

- (void)layoutShutterButtonByOrientation {
    
    // Remove all old constraints
    [_cameraShutter removeFromSuperview];
    [self addSubview:_cameraShutter];
    ...
    if (orientation == UIDeviceOrientationPortrait) {
        ...
        [_cameraShutter.trailingAnchor constraintEqualToAnchor:self.trailingAnchor constant:-20].active = YES;
        [_cameraShutter.centerYAnchor constraintEqualToAnchor:self.centerYAnchor].active = YES;
    }
...
- (void)animateShutterRelease {
    ...
    [UIView animateWithDuration:.1 animations:^{
        ...
        [UIView animateWithDuration:.1 animations:^{
            ...
            weakSelf.animationInProgress = NO; //Enables input manager
        }];
    }];
}
